...
void processLegacy(const QString &iconPath) {
	...
	auto shortcut = appdata + "/Microsoft/Internet Explorer/Quick Launch/User Pinned/TaskBar/Kangram Desktop.lnk";
	if (!QFile::exists(shortcut)) {
		shortcut = appdata + "/Microsoft/Internet Explorer/Quick Launch/User Pinned/TaskBar/Kangram.lnk";
	}
	if (!QFile::exists(shortcut)) {
		return;
	}
...
void processNewShortcuts(const QString &iconPath) {
	...
	const auto shortcuts = {
		path + u"Kangram Desktop/Kangram.lnk"_q,
		path + u"Kangram/Kangram.lnk"_q,
